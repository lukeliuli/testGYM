...
def main():
    ...
    if 1:
        ...
        agent1 = dict(
            ...
            batch_size=300, update_frequency=10, learning_rate=1e-3, multi_step=10,
            ...
            baseline=dict(type='auto', size=64, depth=3),
        ...
        agent2 = dict(
            agent='dqn',
            # Automatically configured network
            network=dict(type='auto', size=128, depth=2), 
            batch_size=64, update_frequency=10, learning_rate=1e-3,
            
            # Reward estimation
            discount=0.99, 
            
            # Save agent every 10 updates and keep the 5 most recent checkpoints
            saver=dict(directory='model', frequency=10, max_checkpoints=10),
            # Log all available Tensorboard summaries
            #summarizer=dict(directory='summaries', summaries='all'),
            # Do not record agent-environment interaction trace
            recorder=None,
        ...
        runner = Runner(agent=agent2, environment=environment, max_episode_timesteps=500)

        # Train for 200 episodes
        save_best_agent = "./best_agent"
        runner.run(num_episodes=1500,save_best_agent=save_best_agent )
        runner.run(num_episodes=100, evaluation=True,save_best_agent=save_best_agent)
        runner.close()
